...
function findMSVC() {
    let msvcPath = process.env["CMAKE_MSVC_PATH"];
    if (msvcPath) {
        Utils.log("Use the MSVC path set by parent cmake project: " + msvcPath);
    } else {
        ...
        msvcPath = path.join(vsPath, "VC");
    }
    if (!fs.existsSync(msvcPath)) {
        Utils.error("The MSVC path is not exist: " + msvcPath);
        return null;
    }
    let vcvars32Path = path.join(msvcPath, "Auxiliary/Build/vcvars32.bat");
    let vcvars64Path = path.join(msvcPath, "Auxiliary/Build/vcvars64.bat");
    if (!fs.existsSync(vcvars64Path) || !fs.existsSync(vcvars32Path)) {
        Utils.error("Could not find \"" + vcvars32Path + "\" or \"" + vcvars64Path + "\"!");
        return null;
    }
    let version = Utils.readFile(path.join(msvcPath, "Auxiliary/Build/Microsoft.VCToolsVersion.default.txt"));
    if(!version) {
        Utils.error("Could not find the version file of MSVC in: " + msvcPath);
        return null;
    }
    version = version.trim();
    return new MSVC(msvcPath, version, vcvars32Path, vcvars64Path);
}
...
function FindNDK() {
    ...
    if (cmakeNDK) {
        ...
        if (version) {
            Utils.log("Use the Android NDK set by parent cmake project: " + cmakeNDK);
    ...
    if (ndkVersions.length > 0) {
        ...
        Utils.error("The minimum required Android NDK version of 20 or higher could not be located! Use the version '" +
...
class WebPlatform extends Platform {
    constructor(debug, verbose) {
        super("web", ["wasm", "wasm-mt"], debug, verbose);
...
function FindDevEcoPath() {
    if (os.platform() === "win32") {
        let programPath = process.env["PROGRAMFILES"];
        let devEcoPath = path.join(programPath, "Huawei/DevEco Studio");
        if (fs.existsSync(devEcoPath)) {
            return devEcoPath;
        }
    }
    if (os.platform() === "darwin") {
        let devEcoPath = '/Applications/DevEco-Studio.app/Contents';
        if (fs.existsSync(devEcoPath)) {
            return devEcoPath;
        }
    }
...
function FindOhosNDK() {
    ...
    if (OhosNDK) {
        ...
        if (version) {
            Utils.log("Use the OHOS NDK set by parent cmake project: " + OhosNDK);
            return {path: OhosNDK, version: version};
        }
    }

    let devEcoPath = FindDevEcoPath();
    if (devEcoPath) {
        let sdkPath = path.join(devEcoPath, "sdk");
        let versionFilePaths = Utils.findFiles(sdkPath, function (filePath) {
...
function CheckCMakeVersion(cmakeCMD) {
    let cmakeVersion = Utils.execSafe(cmakeCMD);
    if (!cmakeVersion) {
        return false;
    }
...
class OhosPlatform extends Platform {
    constructor(debug, verbose) {
        ...
        this.ndkVersion = ndk.version;
        if (!this.ndkHome) {
            Utils.error("Could not find the location of OHOS SDK!");
            process.exit(1);
        }
        if (os.platform() === "win32") {
            this.msvc = findMSVC();
        ...
        let cmakeCMD = "cmake --version";
        if (this.msvc) {
            cmakeCMD = this.msvc.prependVCVars(cmakeCMD, "x64");
        }
        this.useSystemCMake = CheckCMakeVersion(cmakeCMD);
        if (this.useSystemCMake) {
            Utils.log("Using the system cmake to build OHOS project.");
        }
    }

    toolVersion() {
        let info = super.toolVersion();
        info += " OHOS SDK " + this.ndkVersion;
        if (this.msvc) {
            info += " MSVC " + this.msvc.mainVersion;
        }
        return info;
    ...
    getCommandPath(cmd, arch) {
        if (cmd === "cmake") {
            let cmdPath = this.useSystemCMake ? cmd : path.join(this.ndkHome, "build-tools/cmake/bin/cmake");
            cmdPath = Utils.escapeSpace(cmdPath);
            if (this.msvc) {
                return this.msvc.prependVCVars(cmdPath, arch);
            }
            return cmdPath;
        }
