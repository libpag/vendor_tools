...
function FindOhosNDKVersion(ndkPath) {
    ...
    let versionFile = path.join(ndkPath, "native/oh-uni-package.json");
    if (!fs.existsSync(versionFile)) {
        return "";
    }
...
function FindOhosNDK() {
    ...
    if (p === "darwin") {
        ...
        let files = fs.readdirSync(devEcoPath);
        if (files) {
            for (let file of files) {
                let NDK = path.join(devEcoPath, file, 'openharmony');
                if (fs.existsSync(NDK)) {
    ...
    let SDKNative = process.env["OHOS_SDK_NATIVE"];
    if (SDKNative) {
        let NDK = path.dirname(SDKNative);
        let version = FindOhosNDKVersion(NDK);
        if (version) {
            return {path: NDK, version: version};
        }
...
class OhosPlatform extends Platform {
    ...
    getCommandPath(cmd, arch) {
        ...
        let binDir = path.resolve(this.ndkHome, "native/llvm/bin");
        let prefix = "llvm-";
        let cmdPath = path.join(binDir, prefix + cmd);
        cmdPath = getExistedCommandFile(cmdPath);
        if (cmdPath) {
