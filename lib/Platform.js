...
function findMSVC() {
    let vsPath = process.env["CMAKE_VS_PATH"];
    if (vsPath) {
        Utils.log("Use the Visual Studio set by parent cmake project: " + vsPath);
    } else {
        let vswhere = path.resolve(__dirname, "../win/vswhere.exe");
        vsPath = Utils.execSafe(vswhere + " -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath").trim();
    }
    if (!fs.existsSync(vsPath)) {
        Utils.error("The Visual Studio path is not exist: " + vsPath);
        return null;
    }
    ...
    let versionText = Utils.execSafe(Utils.escapeSpace(vcvars64Path));
    let versionLine = versionText.split("\n").find(line => line.includes("Visual Studio"));
    if (!versionLine) {
        Utils.error("Could not find the text line contains Visual Studio in:\n" + versionText);
        return null;
    }
    let vsVersion = versionLine.match(/\d+\.\d+\.\d+/).toString();
    if (!vsVersion) {
        Utils.error("Could not find the version of Visual Studio in:\n" + versionLine);
        return null;
    }
