...
class Vendor {
    ...
    buildVendor(vendorName) {
        ...
        Utils.deletePath(outPath, this.platform.name);
        let script = vendor.script;
        if (script) {
    ...
    publishLibraries(libraryPaths, outPath, xcframework) {
        ...
        if (xcframework) {
            if (hasStaticLibries) {
                libraryTool.createXCFramework(outPath, outPath, true);
            }
            for (let libraryPath of libraryPaths) {
                libraryTool.createXCFramework(libraryPath, outPath, false, false, function (library) {
