...
class Vendor {
    ...
    buildVendor(vendorName) {
        ...
        // Acquire build lock to avoid concurrent builds of the same vendor causing race conditions.
        const lockFilePath = path.join(outPath, '.build.lock');
        const lockFd = Utils.acquireLock(lockFilePath);
        try {
            let sourcePath = vendor.source;
            ...
            Utils.releaseLock(lockFilePath, lockFd);
        }
        return libraryPaths;
    }
