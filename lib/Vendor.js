...
function buildWithScript(platform, sourcePath, outPath, script, deps) {
    ...
    let cmd = Utils.escapeSpace(script.file);
    if (script.executor) {
        if (script.executor === "bash" || script.executor === "sh") {
            // bash and sh need to use / as path separator even on Windows.
            cmd = cmd.split("\\").join("/");
        }
        cmd = script.executor + " " + cmd;
    }
    if (script.arguments) {
    ...
    process.env["VENDOR_ARCHS"] = platform.archs.join(",");
    Utils.log("env: VENDOR_BUILD_TYPE=" + buildType);
    Utils.log("env: VENDOR_OUT_DIR=" + outPath);
    Utils.log("env: VENDOR_ARCHS=" + platform.archs.join(","));
...
class Vendor {
    ...
    buildAll(vendorNames, outPath, xcframework) {
        ...
        if (outPath && libraryPaths.length > 0) {
            ...
            let xcframworkHash = "";
            if (xcframework) {
                let hashFile = path.join(outPath, ".xcframework.md5");
                let cachedHash = Utils.readFile(hashFile);
                let hashes = "";
                for (let arch of this.platform.archs) {
                    hashes += hashTable[arch];
                }
                xcframworkHash = Utils.getHash(hashes);
                if (xcframworkHash === cachedHash) {
                    return;
                }
                Utils.deletePath(hashFile);
            }
            if (archs.length === 0 && !xcframework) {
                return;
            }
            ...
            if (xcframework) {
                let hashFile = path.join(outPath, ".xcframework.md5");
                Utils.writeFile(hashFile, xcframworkHash);
            }
        }
        this.clearVendorOut();
    }
    ...
    buildVendor(vendorName) {
        ...
        // Use a build lock to prevent concurrent builds of the same vendor and to
        // ensure the hash / clean-up logic is executed atomically.
        Utils.withBuildLock(outPath, () => {
            // Re-evaluate which arch(es) actually need rebuilding **after** we hold the lock.
            let archs = [];
            for (let arch of this.platform.archs) {
                const hashFile = path.join(outPath, "." + arch + ".md5");
                const cachedHash = Utils.readFile(hashFile);
            ...
            if (archs.length === 0) {
                // Up-to-date: another process has already produced valid artefacts.
                return;
            }

            Utils.deletePath(path.join(outPath, ".vendor.sha1"));

            const buildType = this.platform.debug ? "debug" : "release";
            const sourcePath = vendor.source;
