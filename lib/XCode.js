...
function FindProductsDir(workspacePath, platform) {
    let sdk = "";
...
function GetCMakeOutputArgs(workspacePath, options) {
    ...
    let releaseDir = "";
    if (options.product) {
        let productsDir = path.resolve(options.product);
        debugDir = path.resolve(productsDir, "Debug");
        releaseDir = path.resolve(productsDir, "Release");
    } else {
        let productsDirs = FindProductsDir(workspacePath, options.platform);
        if (!productsDirs) {
            Utils.error("Can't BUILT_PRODUCTS_DIR in: " + workspacePath);
            process.exit(1);
            return [];
        }
        debugDir = productsDirs.debug;
        releaseDir = productsDirs.release;
    }
...
function Generate(options) {
    ...
    if (workspacePath || options.product) {
