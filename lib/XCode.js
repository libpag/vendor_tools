...
function AddExportPathToScript(config, exportPath) {
    let lines = config.split("\n");
    let results = [];
    let shellScript = "export PATH=\\\"$PATH:" + exportPath + "\\\"\n";
    for (let line of lines) {
...
function Generate(options) {
    ...
    // The homebrew path '/opt/homebrew/bin' is not added to the PATH by default in Xcode.
    let homebrewPath = "/opt/homebrew/bin";
    if (fs.existsSync(homebrewPath)) {
        for (let configFile of configFiles) {
            let config = Utils.readFile(configFile);
            config = AddExportPathToScript(config, homebrewPath);
            Utils.writeFile(configFile, config);
        }
    }
}
