...
function TransformCMakeLists(configFile, platform) {
    ...
    if (configText.indexOf("install(") !== -1 || configText.indexOf("INSTALL(") !== -1) {
        // disable all install()
        const installMacro = "macro (install)\nendmacro ()\n";
        if (configText.indexOf(installMacro) === 0) {
            oldConfigText = configText.substring(installMacro.length);
        } else {
            configText = installMacro + configText;
        }
    }
    if (configText.indexOf("export(") !== -1 || configText.indexOf("EXPORT(") !== -1) {
        // disable all export()
        const exportMacro = "macro (export)\nendmacro ()\n";
        if (configText.indexOf(exportMacro) === 0) {
            oldConfigText = configText.substring(exportMacro.length);
        } else {
            configText = exportMacro + configText;
        }
    }
    if (platform === "web") {
