...
function ParseCMakeVersion(cmakeCMD) {
    let output = Utils.execSafe(cmakeCMD + " --version");
    if (!output) {
        return [0, 0, 0];
    }
    let version = output.match(/\d+\.\d+\.\d+/);
    if (!version) {
        return [0, 0, 0];
    }
...
// Fallback for CMake < 3.15 which does not support CMAKE_PROJECT_INCLUDE.
// Injects the content of vendor.project.cmake directly into CMakeLists.txt after project().
function InjectProjectInclude(configFile) {
    let includeFile = path.resolve(__dirname, "../vendor.project.cmake");
    let includeText = Utils.readFile(includeFile);
    if (!includeText) {
        return "";
    }
    ...
    let endOfLine = configText.indexOf("\n", index);
    if (endOfLine === -1) {
        endOfLine = configText.length;
    }
    let newConfigText = configText.substring(0, endOfLine + 1) + includeText + "\n" + configText.substring(endOfLine + 1);
    Utils.writeFile(configFile, newConfigText);
    return configText;
}
...
class CMake {
    ...
    build(sourcePath, outPath, options, archs) {
        ...
        let useProjectInclude = this.supportsProjectInclude(archs[0]);
        let cmakeConfigFile = path.resolve(sourcePath, "CMakeLists.txt");
        let oldCMakeConfig = "";
        if (!useProjectInclude) {
            oldCMakeConfig = InjectProjectInclude(cmakeConfigFile);
        }
        let libraryMap = {};
        for (let arch of archs) {
            let archBuildPath = path.join(buildPath, arch);
            let cmakeArgs = this.getCMakeArgs(options.arguments, options.deps, archBuildPath, arch, useProjectInclude);
    ...
    supportsProjectInclude(arch) {
        let cmake = this.getCMake(arch);
        let version = ParseCMakeVersion(cmake);
        return version[0] > 3 || (version[0] === 3 && version[1] >= 15);
    }

    getCMakeArgs(args, deps, buildPath, arch, useProjectInclude) {
        let cmakeArgs = [];
        if (useProjectInclude) {
            let projectInclude = path.resolve(__dirname, "../vendor.project.cmake");
            cmakeArgs.push("-DCMAKE_PROJECT_INCLUDE=" + EscapePath(projectInclude));
        }
        cmakeArgs = cmakeArgs.concat(this.getPlatformArgs(arch));
        if (args) {
            // Filter out CMAKE_PROJECT_INCLUDE from user args to prevent conflicts.
            // vendor_tools manages this variable internally via vendor.project.cmake.
            let filteredArgs = args.filter(arg => !arg.includes('-DCMAKE_PROJECT_INCLUDE='));
            cmakeArgs = cmakeArgs.concat(filteredArgs);
