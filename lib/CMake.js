...
function ContainsSharedLibrary(cmakeText) {
    // Match: add_library(<anything including newlines> SHARED <anything>)
    const sharedLibRegex = /^\s*add_library\s*\([\s\S]*?\bSHARED\b[\s\S]*?\)/im;
    return sharedLibRegex.test(cmakeText);
}
...
function TransformCMakeLists(configFile, platform) {
    ...
    if (platform === "web" && ContainsSharedLibrary(configText)) {
...
class CMake {
    ...
    getCMake(arch) {
        let cmakeFromEnv = process.env.CMAKE_COMMAND || process.env.CMAKE;
        if (cmakeFromEnv) {
            return cmakeFromEnv;
        }
