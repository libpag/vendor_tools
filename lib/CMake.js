...
class OhosCMake extends CMake {
    ...
    getCMake(arch) {
        return path.join(this.platform.ndkHome, "native/build-tools/cmake/bin/cmake");
    }

    getPlatformArgs(arch) {
        let ndkHome = this.platform.ndkHome;
        let args = [
            "-DCMAKE_TOOLCHAIN_FILE=" + path.join(ndkHome, "native/build/cmake/ohos.toolchain.cmake"),
            "-DOHOS_PLATFORM=OHOS",
        ];
        switch (arch) {
            case "arm64":
