...
class CMake {
    ...
    build(sourcePath, outPath, options) {
        ...
        if (options.incremental) {
            let cmakeCacheFile = path.resolve(buildPath, "CMakeCache.txt");
            Utils.deletePath(cmakeCacheFile);
        } else {
    ...
    getCMake(arch) {
        let cmake = this.platform.getCommandPath("cmake", arch);
...
class AppleCMake extends CMake {
    ...
    getNativeBuildCommand(target, buildType) {
        return "xcodebuild -target " + target + " -configuration " + buildType + " -jobs " + GetNumberOfJobs();
    }
}
