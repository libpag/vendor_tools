...
class AppleLibraryTool extends LibraryTool {
    ...
    createXCFramework(libraryPath, outPath) {
        if (!outPath) {
            outPath = libraryPath;
        }
        ...
        for (let library of libraries) {
            ...
            for (let key of archKeys) {
                ...
                let fatLibrary = path.join(libraryPath, key, libraryName);
                this.createFatLibrary(libraryFiles, fatLibrary);
            ...
            for (let fatLibrary of fatLibraries) {
                Utils.deletePath(fatLibrary);
                Utils.deleteEmptyDir(path.dirname(fatLibrary));
            }
        }
    }
