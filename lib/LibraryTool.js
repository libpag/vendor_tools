...
class WinLibraryTool extends LibraryTool {
    mergeLibraries(libraries, output, arch) {
        ...
        // Use response file to avoid command line too long error
        let responseFile = output + ".rsp";
        let responseContent = list.join("\n");
        Utils.writeFile(responseFile, responseContent);

        let cmd = libTool + " /out:" + Utils.escapeSpace(output) + " @" + Utils.escapeSpace(responseFile);
        Utils.exec(cmd, process.cwd(), this.platform.verbose);

        // Clean up response file
        Utils.deletePath(responseFile);
    }
}
