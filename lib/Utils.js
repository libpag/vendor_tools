...
function sleep(ms) {
    // Improved sleep function with multiple fallback strategies
    try {
        // Try Atomics.wait first (most efficient)
        Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms);
    } catch (e) {
        // Fallback to sync sleep using busy wait (less efficient but works everywhere)
        ...
        try {
            ...
            if (err.code === 'EEXIST') {
                ...
                try {
                    ...
                    // Check if the lock is stale (older than 30 minutes or from dead process)
                ...
                waitForLockRelease(lockFile, timeoutMs, startTime);
